    ...
    <div className='h-full w-full bg-zinc-900 p-8'>
        <div className='searchbar-id h-full flex flex-col items-center '>
            <div className='flex items-center gap-0 w-full justify-center'>
              <input type="text" name="name" id="searchid" placeholder='SEARCH' className='rounded-2xl h-12 w-1/3 text-xl font-semibold text-zinc-950 mb-6' onChange={e=>search(e)}/>
            ...
            </div>
            {
              users.map((user, index)=>{           
                return <div key={index} className='userid h-20 w-2/3 flex justify-startx items-center gap-10 p-2 bg-zinc-500 text-white'>
                    <div className='avatar rounded-full border-2 border-solid border-red-600 object-cover h-full w-13'><img src={`${user.profileimage}`} alt="" className='object-cover h-full w-full rounded-full'/></div>
